...
public class ConsultaService {
    ...
    //EspecialistaPj
    public void createConsultaComPacienteEspPj(ConsultaComPacienteEEspecialistaPj dto){
        Consulta consulta = new Consulta();

        Optional<Paciente> pacienteExist = pacienteRepository.findById(dto.getIdPaciente());
        if(pacienteExist.isEmpty()) throw new Error("User not found");

        Optional<EspecialistaPj> espPjExist = especialistaPjRepository.findById(dto.getIdEspecialista());
        if(espPjExist.isEmpty()) throw new Error("User not found");

        Paciente paciente = pacienteExist.get();
        EspecialistaPj espePj = espPjExist.get();

        consulta.setPaciente(paciente);
        consulta.setEspePj(espePj);
        consulta.setValorConfirmado(dto.getValorConfirmado());
        consulta.setDtConsulta(dto.getDtConsulta());
